...
class AddCommentSpec {
    ...
    fun setup() {
        ...
        addComment = AddComment(repository, executor)
    }

    @Test
    fun `given a response when executes it should completes`() {
        every { repository.addComment(any()) } returns Completable.complete()

        val result = addComment.buildUseCase(AddComment.Param(CommentDataFactory.get())).test()
